...
# Make the project importable no matter where Streamlit launches the script
# from. Walk upward until we find the directory that actually contains the
# `config` package (the project root), then put it on sys.path.
_here = Path(__file__).resolve()
for _candidate in (_here.parent, *_here.parents):
    if (_candidate / "config" / "settings.py").is_file():
        sys.path.insert(0, str(_candidate))
        break
else:  # last resort: the file's own parent, so the error is at least clear
